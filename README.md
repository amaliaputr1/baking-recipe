...
#🍞 #Bakery Recipes App# A full‑stack web application for managing and browsing bakery recipes. Built with PHP (XAMPP) for backend and React + TailwindCSS (Vite) for frontend.
#📂 Project Structure backend/ → PHP backend (API, database, seed data)

frontend/ → React frontend (UI, image upload/preview, CRUD)

recipes.sql → Database seed file with bakery recipes

#🍰 Features Recipe CRUD: Add, edit, delete, and view bakery recipes
...
#📦 Seed Data File recipes.sql sudah berisi beberapa resep bakery (roti, cake, pastry). Reviewer bisa langsung mencoba tanpa input manual.
